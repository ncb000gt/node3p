...
Node3p.prototype.skipped = function(filename, primary, album, title, reason) {
  sys.puts('Skipped download: "'+title+'" by "'+ primary+'" to "'+filename+'". Reason: ' + reason);
  this.downloaded.push({filename: filename, primary: primary, album: album, title: title});
...
Node3p.prototype.getFile = function(uri, primary, album, title, left) {
  ...
  if (path.existsSync(filepath)) {
    self.dlCount--;
    self.emit('skipped', filepath, primary, album, title, "Already have the file.");
    return;
  }
